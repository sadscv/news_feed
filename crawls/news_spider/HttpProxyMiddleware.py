#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 17-11-13 下午11:11
# @Author  : sadscv
# @File    : HTTPProxyMiddleware.py
...
class HttpProxyMiddleware(object):
    ...
    def inc_proxy_index(self, current=-1):
        ...
        if self.proxy_index == 0 and \
                        datetime.now() < self.last_no_proxy_time + timedelta(minutes=2):
            logger.info("captcha thrashing")
            self.fetch_new_proxyes()

        # 如果代理列表中有效的代理不足的话重置为valid
        if self.len_valid_proxy() <= self.fixed_proxy or\
    ...
    '''
    通常情况下Request对象由Spider们生成,接着在整个Scrapy框架内被传递至Downloader,
    此处Downloader会对request做出处理,并返回三个Response对象给该request对应的spider。
